...
-- Criar função para verificar se uma categoria existe
CREATE OR REPLACE FUNCTION category_exists(category_name text, category_type text DEFAULT 'AMBOS')
RETURNS boolean AS $$
BEGIN
    IF category_type = 'AMBOS' THEN
        RETURN EXISTS (
...
-- Política para permitir que usuários vejam categorias padrão mesmo sem estar logados
DROP POLICY IF EXISTS "Allow public access to default categories" ON categories;
CREATE POLICY "Allow public access to default categories" ON categories
