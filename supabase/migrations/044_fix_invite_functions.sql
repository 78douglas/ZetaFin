-- Remover função existente e recriar com nova assinatura
DROP FUNCTION IF EXISTS validate_invite_token(TEXT);

-- Melhorar função de criação de convite
...
-- Recriar função de validação com nova assinatura
...
-- Comentários
COMMENT ON FUNCTION create_invite_token(TEXT) IS 'Cria um novo token de convite com melhor tratamento de erros e logging';
COMMENT ON FUNCTION validate_invite_token(TEXT) IS 'Valida um token de convite e retorna informações detalhadas';